<template>
  <div>
    <div v-if="!$common.isEmpty(department)">
      <!-- 封面 -->
      <div class="department-head my-animation-slide-top">
        <!-- 背景图片 -->
        <el-image class="department-image my-el-image"
                  v-once
                  lazy
        ...
        <div class="department-info-container">
          ...
          <div class="department-info">
            ...
            <span>&nbsp;{{ department.viewCount }} 热度</span>
            <span>·</span>
            <svg viewBox="0 0 1024 1024" width="14" height="14" style="vertical-align: -2px;">
              <path
            ...
            <span>&nbsp;{{ department.commentCount }} 条评论</span>
            <span>·</span>
            <svg viewBox="0 0 1024 1024" width="14" height="14" style="vertical-align: -2px;">
              <path
            ...
            <span>&nbsp;{{ department.likeCount }} 点赞</span>
          </div>
        </div>

<!--        <div class="department-info-news"-->
<!--             @click="weiYanDialogVisible = true"-->
<!--             v-if="!$common.isEmpty($store.state.currentUser) && $store.state.currentUser.id === department.userId">-->
...
<!--        </div>-->
      ...
      <div style="background: var(--background);display: flex">
        <div class="department-container my-animation-slide-bottom">
          <div v-if="!$common.isEmpty(department.videoUrl)" style="margin-bottom: 20px">
            <videoPlayer :url="{src: $common.decrypt(department.videoUrl)}"
                         :cover="department.departmentCover">
            </videoPlayer>
          </div>
          ...
          <div class="department-update-time">
            <span>文章更新于 {{ department.updateTime }}</span>
          </div>
          <!-- 分类 -->
<!--          <div class="department-sort">-->
<!--            <span @click="$router.push({path: '/sort', query: {sortId: department.sortId, labelId: department.labelId}})">{{ department.sortDepartment.sortName +" ▶ "+ department.label.labelName}}</span>-->
<!--          </div>-->

          <!-- 评论 -->
          <div v-if="department.commentStatus === true">
            <comment :type="'department'" :source="department.id" :userId="department.userId"></comment>
          </div>
        </div>
...
<style scoped>
  ...
  .department-info span:not(:last-child) {
    margin-right: 5px;
  }
