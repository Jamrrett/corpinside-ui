      ...
      <div class="in-up" id="loginAndRegist">
        <div class="form-container sign-up-container">
          <div class="myCenter">
            ...
            <div style="width: 100%;height: auto;display: flex;flex-direction: row;padding: 0">
              <input v-model="code" type="text" placeholder="验证码" :disabled="isCodeInputDisabled" style="width: 70%;border-radius: 5px 0 0 5px">
              <a style="width: 30%;height: auto;color: var(--white);background: var(--lightRed);border-radius: 0 5px 5px 0;border: none;padding: 12px 15px;margin: 10px 0;text-align: center"
                 @click="sendCode"
                 :disabled="isSendCodeDisabled"
                 :style="{
                    cursor: isSendCodeDisabled ? 'not-allowed' : 'pointer',
                    opacity: isSendCodeDisabled ? 0.5 : 1
                  }">
                <span>{{ countdown }}</span>
              </a>
            </div>
...
export default {
  ...
  created() {
    ...
    this.redirect = sessionStorage.getItem('redirectPath').startsWith('/') ? sessionStorage.getItem('redirectPath') : '/';
  },
  methods: {
    addPicture(res) {
      this.avatar = res;
    ...
    sendCode() {
      ...
      this.$http.get(this.$constant.baseURL + '/user/getCodeForForgetPassword', params)
        .then((res) => {
          this.$message({
